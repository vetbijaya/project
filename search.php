...
<head>
    ...
    <title>Antibiotics Type - Search</title>
</head>
<body>
<h3>Antibiotics Type - Search Results</h3>

<?php
// This function builds a search query from the search keywords and sort setting
function build_query($user_search, $sort)
{
    $search_query = "SELECT * FROM antibiotic_type";
    ...
    switch ($sort) {
        // Ascending by antibiotic name
        case 1:
            $search_query .= " ORDER BY antibiotic_name";
            break;
        // Descending by antibiotic name
        case 2:
            $search_query .= " ORDER BY antibiotic_name DESC";
            break;
        // Ascending by company
        case 3:
            $search_query .= " ORDER BY company";
            break;
        // Descending by company
        case 4:
            $search_query .= " ORDER BY company DESC";
            break;
...
function generate_sort_links($user_search, $sort) {
    ...
    switch ($sort) {
        case 1:
            $sort_links .= '<td><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=2">Antibiotic_name</a></td><td>Description</td>';
            $sort_links .= '<td><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=3">Company</a></td>';
            $sort_links .= '<td><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=5">Date Posted</a></td>';
            break;
        case 3:
            $sort_links .= '<td><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=1">Antibiotic_name</a></td><td>Description</td>';
            $sort_links .= '<td><a href = "' . $_SERVER['PHP_SELF'] . '?usersearch=' . $user_search . '&sort=4">Company</a></td>';
...
while ($row = mysqli_fetch_array($result)) {
    ...
    echo '<td valign="top" width="20%">' . $row['antibiotic_name'] . '</td>';
    echo '<td valign="top" width="50%">' . substr($row['description'], 0, 100) . '...</td>';
    echo '<td valign="top" width="10%">' . $row['company'] . '</td>';
